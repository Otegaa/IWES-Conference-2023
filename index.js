const hamburger = {
  container: document.querySelector('.hamburger-container__img'),
  close: document.querySelector('.hamburger__close-img'),
  menu: document.querySelector('.hamburger'),
  menuLinks: document.querySelectorAll('.menu-links'),
};

const speakers = {
  container: document.querySelector('.featured-speakers__dynamic-box'),
  dynamicSpeakers: [
    {
      id: 1,
    ...
    },
    {
      ...
      description:
        'Blessing is one of the fastest rising stars in tech. She has been a major part of the progress of many companies in Africa and Europe.',
    },
  ],
  currentItems: 2,
  seeMore: document.querySelector('.see-more__btn'),
  seeLess: document.querySelector('.see-less'),
};

// Opening and Closing the hamburger menu
const openHamburgerMenu = () => {
  hamburger.menu.classList.remove('hidden');
};
...
hamburger.container.addEventListener('click', openHamburgerMenu);
hamburger.close.addEventListener('click', closeHamburgerMenu);

// Go to the link
hamburger.menuLinks.forEach((link) => {
